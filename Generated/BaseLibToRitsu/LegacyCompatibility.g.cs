...
public abstract class CustomCharacterModel : CharacterModel, ICustomModel, ILocalizationProvider, ISceneConversions
{
    ...
    public void RegisterSceneConversions()
    {
        ...
        if (!string.IsNullOrWhiteSpace(CustomEnergyCounterPath))
        {
            LegacyNodeFactory.RegisterSceneType<NEnergyCounter>(CustomEnergyCounterPath);
        }
...
public static class LegacyCompatibilityBootstrap
{
    ...
    private static readonly FieldInfo? EnergyCounterPlayerField = AccessTools.Field(typeof(NEnergyCounter), "_player");
    ...
    internal static TNode? TryCreateSceneNode<TNode>(string? scenePath, Type ownerType, string sourceMemberName)
        where TNode : Node, new()
    {
        ...
        try
        {
            return LegacyNodeFactory.CreateFromScenePath<TNode>(scenePath);
        }
        catch (Exception ex)
        {
            Logger.Warn(
                $"Failed to convert scene '{scenePath}' to {typeof(TNode).Name} for '{ownerType.FullName}' via '{sourceMemberName}': {ex.GetBaseException().Message}");
            return null;
        }
    }

    internal static void BindEnergyCounterPlayer(NEnergyCounter counter, Player player)
    {
        EnergyCounterPlayerField?.SetValue(counter, player);
    }
    ...
    private static IEnumerable<Type> GetPatchTypes()
    {
        ...
        yield return typeof(CharacterEnergyCounterCreatePatch);
    ...
    private static class MonsterCreateVisualsPatch
    {
        ...
        private static bool Prefix(MonsterModel __instance, ref NCreatureVisuals? __result)
        {
            ...
            if (__result == null && !string.IsNullOrWhiteSpace(monster.CustomVisualPath))
            {
                __result = LegacyCompatibilityBootstrap.TryCreateSceneNode<NCreatureVisuals>(
                    monster.CustomVisualPath,
                    monster.GetType(),
                    nameof(CustomMonsterModel.CustomVisualPath));
            }
    ...
    private static class CharacterCreateVisualsPatch
    {
        ...
        private static bool Prefix(CharacterModel __instance, ref NCreatureVisuals? __result)
        {
            ...
            if (__result == null && !string.IsNullOrWhiteSpace(character.CustomVisualPath))
            {
                __result = LegacyCompatibilityBootstrap.TryCreateSceneNode<NCreatureVisuals>(
                    character.CustomVisualPath,
                    character.GetType(),
                    nameof(CustomCharacterModel.CustomVisualPath));
            }
    ...
    private static class CharacterEnergyCounterCreatePatch
    {
        [HarmonyPrefix]
        private static bool Prefix(Player player, ref NEnergyCounter? __result)
        {
            if (MegaCrit.Sts2.Core.TestSupport.TestMode.IsOn ||
                player.Character is not CustomCharacterModel character ||
                string.IsNullOrWhiteSpace(character.CustomEnergyCounterPath))
            {
                return true;
            }

            var counter = LegacyCompatibilityBootstrap.TryCreateSceneNode<NEnergyCounter>(
                character.CustomEnergyCounterPath,
                character.GetType(),
                nameof(CustomCharacterModel.CustomEnergyCounterPath));
            ...
            LegacyCompatibilityBootstrap.BindEnergyCounterPlayer(counter, player);
            __result = counter;
            return false;
        }
    }
